...
An implementation of `vptr` must provide the following static members:
...
In addition to the requirements of `vptr`, an implementation of `extern_vptr`
must have the following static members:
...
```c++
struct indirect_extern_vptr : virtual extern_vptr {};
```

### Description

`indirect_extern_vptr` makes `virtual_ptr`{empty}s use pointers to pointers to a
v-table, instead of straight pointers. As a consequence, they remain valid after
