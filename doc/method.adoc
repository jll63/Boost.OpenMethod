...
has a type in the form `virtual_ptr<T, Policy>` or `virtual_<T>`. The dynamic
types of the virtual arguments (the arguments corresponding to virtual
...
The dispatch mechanism determines the v-table for each virtual argument (`arg`)
as follows:

1. Call `virtual_traits::rarg(arg)`. The result is the _resolver_ _argument_, or
   `rarg.`

2. If `rarg` is a `virtual_ptr`, it contains he pointer to the v-table.

3. If `rarg` has a public `boost_openmethod_vptr` member variable, it is
   a pointer to the v-table, or a pointer to a pointer to the v-table, depending
   on the `Policy`.

4. Otherwise, call `Policy::dynamic_vptr(rarg)`.
