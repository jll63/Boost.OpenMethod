...
1. Call `virtual_traits::peek(arg)`. The result is the _resolver_ _argument_, or
   `peek.`

2. If `peek` is a `virtual_ptr`, it contains the pointer to the v-table.

3. If `peek` has a public `boost_openmethod_vptr` member variable, it is
   a pointer to the v-table, or a pointer to a pointer to the v-table, depending
   on the `Policy`.

4. Otherwise, call `Policy::dynamic_vptr(peek)`.
