...
2. If `boost_openmethod_vptr(result)` can be evaluated, call it.

3. If `peek` has a public `boost_openmethod_vptr` member variable, it is assumed
   to contain a pointer to the v-table, or a pointer to a pointer to the
   v-table, depending on the `Policy`.

4. Call `Policy::dynamic_vptr(result)`.
