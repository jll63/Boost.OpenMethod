...
`fork` replaces all the facet implementations in the form `template<class
Policy, typename... T> class FacetImplementation` with
`FacetImplementation<NewPolicy, T...>`. All the other facets implementations are
copied as is.
