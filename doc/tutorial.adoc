...
for open-methods is to call `next`, a funcxtion that is available only inside
the body of an overrider.
...
`next` automatically selects the next most specialized overrider - i.e. the
overrider that would have been called if this overrider did not exist.
...
[source,c++]
----
include::{examplesdir}/hello_world.cpp[tag=main1]
----

Finally, `poke` is called like an ordinary function (which it is), passing it
the animals by reference. `virtual_ptr`

[source,c++]
----
include::{examplesdir}/hello_world.cpp[tag=main2]
...
}
----
...
[source,c++]
----
include::{examplesdir}/hello_world.cpp[tag=multi_method]
----
[source,c++]
----
include::{examplesdir}/hello_world.cpp[tag=initialize]
----
