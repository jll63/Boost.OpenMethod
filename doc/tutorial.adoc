...
a `Animal&`; and it calls the method. The construction of the `virtual_ptr` is
the costly part, as it involves a hash table lookup. Once that price is paid,
the `virtual_ptr` can be used multiple times. It is passed to the overrider,
which can further method calls through it. It can be stored in variables in
place of straight pointers.
...
The next two instructions are the method call proper. Only two instructions.
According to llvm-mca, they take one cycle - the same as a native virtual
function call.
...
`virtual_ptr` can also be used in combined with smart pointers.
...
parameters with a type decorated with `virtual_` are are considered in overrider
selection (along with any `virtual_ptr` parameters).
...
include::{examplesdir}/virtual_.cpp[tag=call_ref,indent=0]
----

Note that `virtual_` is not used in the overriders. It is also removed from the
method's signature.
...
However, we can now customize how the vptr is obtained. When the call mechanism
sees a virtual argument that is not a `virtual_ptr`, it looks for a function
called `boost_openmethod_vptr`, that takes the argument (by const reference),
and returns a `vptr_type`. If one is found, it is used to obtain the vptr. The
vptr for registered class can be obtained via a variable template
`static_vptr`, nested in class `default_policy` (more on policies below).
...
[source,c++]
----
include::{examplesdir}/virtual_.cpp[tag=set_vptr,indent=0]
----
